...
export function getPair(): [number, number] {
  const numNotes = notes.length;
  const firstNoteIndex = randIntBetween(
    MAX_DISTANCE_SEMITONES,
    numNotes - MAX_DISTANCE_SEMITONES
  );
  const intervalSemitones = randIntBetween(
    -MAX_DISTANCE_SEMITONES,
    MAX_DISTANCE_SEMITONES
  );
